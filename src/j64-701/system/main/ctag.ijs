...
NB. =========================================================
cleartags=: 3 : 0
tagkludge=: 0 [ tagtags=: tagfiles=: taglines=: ''
EMPTY
)
...
if. #y do.
  ...
  if. #Tags_j_ do. parsetags"0 jpath&.> boxopen Tags_j_ end.
end.
EMPTY
)
...
taglines=: taglines, ; <@(,&({.a.))@(_3&}.)@}.@}.@;@:(TAB&,&.>)@}:@(2&}.)@(<;._2)@(,&TAB);._2 a
NB. !!! will be removed once ctags.exe can generate tags with exclude working
NB. exclude .snp/.svn/release for tags generated by windows ctags.exe
NB. files under .git do not have .ijs as file extension
if. -.tagkludge do.    NB. short circuit
  tagkludge=: tagkludge +. (1 e. ('/.snp/'&E. +. '/.svn/'&E. +. '/release/'&E.)) b
end.
EMPTY
)
...
if. tagkludge *. 0~:#res do.
  res=. (#~ (1 -.@e. ('/.snp/'&E. +. '/.svn/'&E. +. '/release/'&E.))@tagfile"0) res
end.
~.res
)
...
nms=. 0 : 0
cleartags loadtags ta tagcp tagopen tagselect
)

nms=. (nms e.' ',LF) <;._2 nms
". > nms ,each (<'_z_=:') ,each nms ,each <'_ctag_'
